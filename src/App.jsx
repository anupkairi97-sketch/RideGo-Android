...
import {
  ...
  Menu, Gift, Share2, MoreHorizontal, Headphones, CheckCircle2, Mail, Briefcase, RefreshCw,
} from "lucide-react";
  ...
  return (
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      /* Smooth live-marker movement: Leaflet positions markers via CSS
         transform, so animating that property makes the driver's car
         icon glide to its new position instead of jumping. */
      .leaflet-marker-icon { transition: transform 0.7s linear; }
      .leaflet-pane .leaflet-marker-icon.rg-no-anim { transition: none; }
    `}</style>
  );
}

/* ------------------------------ Brand mark ----------------------------- */
// Custom RideGo logo: a sharp forward-motion arrow (matches the app icon).
// Rendered in white on the app's crimson badge everywhere in-app.
const RideGoMark = memo(function RideGoMark({ size = 30, color = "#fff" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="RideGo">
      <path d="M12 78 L82 8 L70 46 L92 60 L46 66 L36 92 L28 62 Z" fill={color} />
    </svg>
  );
});
  ...
  return (
    <div className="flex flex-col h-full px-6 rg-anim-in">
      ...
      <div className="pb-8 flex flex-col gap-3">
        ...
        <Btn disabled={mobile.length !== 10 || sending} onClick={handleSendOtp}>
          {sending ? <Loader2 size={17} className="animate-spin" /> : <>{t.sendOtp} <ChevronRight size={17} /></>}
        </Btn>
...
function POtpScreen({ go, params }) {
  ...
  const handleVerify = async () => {
    ...
    setVerifying(true);
    try {
      await verifyCustomOtp(params.mobile, otp.join(""));
      go("register", params);
    } catch (e) {
  ...
  const handleResend = async () => {
    ...
    setResending(true);
    try {
      await sendCustomOtp(params.mobile);
    } catch (e) {
  ...
  return (
    <div className="flex flex-col h-full px-6 rg-anim-in">
      ...
      <div className="flex-1 flex flex-col justify-center gap-4">
        ...
        {error && <p className="text-xs" style={{ color: "var(--red)" }}>{error}</p>}
        <button onClick={handleResend} disabled={resending} className="text-sm font-semibold text-left" style={{ color: "var(--accent)" }}>
          {resending ? "Sending…" : t.resend}
        </button>
      </div>
      <div className="pb-8">
        <Btn disabled={!complete || verifying} onClick={handleVerify}>
          {verifying ? <Loader2 size={17} className="animate-spin" /> : t.verify}
        </Btn>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full rg-anim-in relative">
      ...
      <div className="flex-1 overflow-y-auto rg-scroll px-5 pt-2 pb-4">
        ...
        <div className="rounded-2xl p-4 mb-4 rg-card relative">
          ...
          <div className="absolute left-[27px] top-[34px] bottom-[calc(50%-2px)] w-0 border-l-2 border-dotted" style={{ borderColor: "var(--border)" }} />
          <div className="flex items-start gap-2.5 pt-3">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-transform active:rotate-180"
    ...
    <div className="flex flex-col h-full rg-anim-in">
      ...
      <div className="flex-1 overflow-y-auto rg-scroll px-6 pt-2 flex flex-col gap-2.5">
        {VEHICLES.map((v, i) => {
          ...
          const cheapest = i === VEHICLES.reduce((mi, x, xi) => fareFor(x, realDistanceKm) < fareFor(VEHICLES[mi], realDistanceKm) ? xi : mi, 0);
          return (
            <button key={v.id} onClick={() => setSelected(v.id)} className="relative flex items-center gap-3.5 rounded-2xl p-3.5 text-left transition-all"
              style={{
                background: active ? "var(--accent-tint)" : "var(--surface)",
                border: `1.5px solid ${active ? "var(--accent)" : "var(--border)"}`,
                boxShadow: active ? "0 14px 28px -14px var(--accent)" : "0 2px 8px -4px rgba(0,0,0,0.06)",
                transform: active ? "scale(1.01)" : "scale(1)",
              }}>
              <div className="w-13 h-13 rounded-2xl flex items-center justify-center shrink-0" style={{ width: 52, height: 52, background: active ? "var(--accent-grad)" : "var(--surface-2)", boxShadow: active ? "0 8px 16px -8px var(--accent)" : "none" }}>
                <v.Icon size={24} style={{ color: active ? "#fff" : "var(--accent)" }} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <p className="font-semibold text-[15px]">{v.name}</p>
                  {cheapest && (
                    <span className="text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-full" style={{ background: "#DCFCE7", color: "#15803D" }}>Cheapest</span>
                  )}
                </div>
                <p className="text-xs flex items-center gap-1 mt-0.5" style={{ color: "var(--muted)" }}><Clock size={11} /> {v.eta} min away · {v.capacity} {t.capacity}</p>
              </div>
              <div className="flex flex-col items-end gap-1 shrink-0">
                <p className="rg-display font-bold text-lg">₹{fareFor(v, realDistanceKm)}</p>
                <div className="w-5 h-5 rounded-full flex items-center justify-center" style={{ border: `2px solid ${active ? "var(--accent)" : "var(--border)"}`, background: active ? "var(--accent)" : "transparent" }}>
...
function PInTripScreen({ go, params }) {
  ...
  const totalTripKm = params.distanceKm || DISTANCE_KM;
  const tripProgressPct = remainingKm != null && totalTripKm > 0
    ? Math.min(100, Math.max(0, ((totalTripKm - remainingKm) / totalTripKm) * 100))
    : 0;
  ...
  if (!tripCompleted) {
    return (
      <div className="flex flex-col h-full rg-anim-in">
        ...
        <div className="px-5">
          <div className="rounded-2xl p-4 mb-3" style={{ background: "var(--accent-grad)" }}>
            ...
            {remainingKm != null ? (
              <>
                <div className="flex items-end justify-between mt-2">
                  <div>
                    <p className="rg-display text-3xl font-bold text-white leading-none">{formatDistance(remainingKm)}</p>
                    <p className="text-[11px] mt-1" style={{ color: "rgba(255,255,255,0.75)" }}>remaining</p>
                  </div>
                  <div className="text-right">
                    <p className="rg-display text-3xl font-bold text-white leading-none">{remainingEta ? formatDuration(remainingEta) : "—"}</p>
                    <p className="text-[11px] mt-1" style={{ color: "rgba(255,255,255,0.75)" }}>to destination</p>
                  </div>
                </div>
                <div className="mt-3 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.25)" }}>
                  <div className="h-full rounded-full" style={{ width: `${tripProgressPct}%`, background: "#fff", transition: "width 0.8s ease" }} />
                </div>
                <div className="flex items-center justify-between mt-2 text-[10px] font-semibold" style={{ color: "rgba(255,255,255,0.8)" }}>
                  <span>Picked up</span>
                  <span style={{ opacity: tripProgressPct > 15 && tripProgressPct < 90 ? 1 : 0.55 }}>En route</span>
                  <span style={{ opacity: tripProgressPct >= 90 ? 1 : 0.55 }}>Arriving</span>
                </div>
              </>
            ) : (
              <p className="text-xs mt-2" style={{ color: "rgba(255,255,255,0.9)" }}>{t.enjoyRide}</p>
            )}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full px-6 rg-anim-in">
      ...
      <div className="pb-8 flex flex-col gap-3">
        ...
        {error && <p className="text-xs" style={{ color: "var(--red)" }}>{error}</p>}
        <Btn disabled={mobile.length !== 10 || sending} onClick={handleSendOtp}>
          {sending ? <Loader2 size={17} className="animate-spin" /> : <>Send OTP <ChevronRight size={17} /></>}
        </Btn>
      </div>
    </div>
  );
...
function DOtpScreen({ go, params }) {
  ...
  return (
    <div className="flex flex-col h-full px-6 rg-anim-in">
      ...
      <div className="pb-8">
        <Btn disabled={!complete || verifying} onClick={handleVerify}>
          {verifying ? <Loader2 size={17} className="animate-spin" /> : "Verify & continue"}
        </Btn>
      </div>
    </div>
  );
